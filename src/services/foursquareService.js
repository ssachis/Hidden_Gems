...
export const getFoursquarePlaces = async (cityId, customApiKey = null) => {
  ...
  if (!customApiKey || customApiKey === 'enter api key') {
    ...
    try {
      ...
      rawPlaces = response.data.results.map((item) => {
        ...
        return {
          ...
          description: item.description || `A highly rated local spot situated in the heart of the city. Discover its authentic architecture and local surroundings.`,
